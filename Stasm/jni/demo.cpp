...
#include "face_morpher/init.hpp"
#include "face_morpher/morpher.hpp"
...
JNIEXPORT jintArray JNICALL Java_org_stasmdemo_android_MainActivity_findFaceLandmarks(
	...
	static const char* path =
			"/data/data/org.stasmdemo.android/app_stasm/testface.jpg";
	...
	if (!mGray->data) {
		__android_log_print(ANDROID_LOG_INFO, "Stasm", "Cannot load image",
				path);
	...
	if (!stasm_search_single(&foundface, landmarks, (const char*) mGray->data,
		...
		__android_log_print(ANDROID_LOG_ERROR, "Stasm",
				"Error in stasm_search_single %s", stasm_lasterr());
	...
	aprintf("landmarks len : %i ptr : %i", len(landmarks), landmarks);
	...
	if (!foundface) {
		...
		__android_log_print(ANDROID_LOG_DEBUG, "Stasm Time Cost",
				"Time:%.3fsec", asmTime);
...
JNIEXPORT jlong JNICALL Java_com_bestlove_morpher_MFLib_morpher(JNIEnv* env,
		jobject obj, jlong img1ptr, jlong img2ptr) {
	aprintf("call in jni");
	cv::Mat* img1 = (cv::Mat*) img1ptr;
	aprintf("reset mat 1 %i %i", img1->cols, img1->rows);
	cv::Mat* img2 = (cv::Mat*) img2ptr;
	aprintf("reset mat 2 %i %i", img2->cols, img2->rows);
	cv::Mat* res = morpher::morpher(img1, img2);
	aprintf("res mat 3 %i %i", res->cols, res->rows);
	aprintf("call out jni");
	return (jlong) (res);
}
}
